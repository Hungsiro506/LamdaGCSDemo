package com.gcs.batchcore.batchlayer.hbase;
 ...
 * Hbase Service.
 * 
 * @author phivt [phivt@gcs-vn.com]
 * @contributor khuongav [khuongav@gcs-vn.com]
 * @since 7-8-2015
 */
public class HBaseService {
    ...
    public HBaseService(String hostname) throws IOException {
        ...
        conf.set(Constants.HBASE_CONFIGURATION_ZOOKEEPER_CLIENTPORT, Constants.HBASE_ZOOKEEPER_QUORUM_PORT);
    }
    ...
    /**
     * Initialization a table with a column Family.
     * 
     * @param strTableName
     *            Table Name
     * @param colF
    ...
    public HTable createTableLoadDatafromFile(String strTableName, String[] colF) throws IOException {
        // If Table not exists
        TableName tableName = TableName.valueOf(strTableName);
        {
            LOG.debug("Create a Table: " + strTableName);
    ...
    public void insertInto(HTable table, File file, String colF) {
        try {
            byte[] content = Bytes.toBytes(FileUtils.readFileToString(file, "UTF-8"));
            this.insert(table, file.getName(), colF, "", content);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    @SuppressWarnings("deprecation")
    private void insert(HTable table, String rowKey, String prefix, String qualifier, byte[] value) throws IOException {
    ...
    public void addData(String strTableName, String colF, DataHBase[] data) throws IOException {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int icol = 0; icol < data[i].getColName().size(); icol++) {
                put.addColumn(Bytes.toBytes(colF), data[i].getColName().get(icol), data[i].getColData().get(icol));
            }
            table.put(put);
        }
        table.close();
     ...
     *            destination table
     * @param rowKey
     *            row key on source table
     * @param relationRow
     *            rowKey on destination table
     * @throws IOException
     *             I/O Exception
     */
    public void addRelationData(TableName sourceTable, String destTable, String rowKey, String[] relationRow)
            throws IOException {
     ...
     * 
     * @param tableName
     *            the table name on database
     * @param rowKey
     *            the row key of data on database
     * @return {@link Result}
     * @throws IOException
     *             {@link IOException}
     */
     ...
     *            the row key of data on database
     * @param colF
     *            the column family of data on database
     * @return {@link Result}
     * @throws IOException
     *             {@link IOException}
     */
     ...
     *            the column family of data on database
     * @param maxVersions
     *            the number of versions of data on database
     * @return {@link Result}
     * @throws IOException
     *             {@link IOException}
     */
    public Result get(String tableName, String rowKey, String colF, int maxVersions) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        Get get = new Get(Bytes.toBytes(rowKey));
        get.addFamily(Bytes.toBytes(colF));
